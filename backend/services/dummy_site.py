...
def build_llms_txt(pages: list[dict], base_url: str) -> str:
    """LLM-citable site index (llms.txt spec) for the generated mirror."""
    lines = [
        "# ZuiGO Engine Mirror",
        "",
        f"> {base_url} - mirror generated by ZuiGO Engine with approved SEO changes applied.",
        "",
        "## Pages",
        "",
    ]
    for p in pages:
        ...
        title = (p.get("title") or "").strip() or norm
        lines.append(f"- [{title}]({norm})")
        if p.get("meta_description"):
            lines.append(f"  - Description: {p['meta_description'].strip()[:200]}")
    lines.append("")
    return "\n".join(lines)
...
async def generate_dummy_site(job_id: str, auto_compare: bool = False) -> dict:
    ...
    llms_path = os.path.join(target_dir, "llms.txt")
    with open(llms_path, "w", encoding="utf-8") as f:
        f.write(build_llms_txt(pages, base_url))
