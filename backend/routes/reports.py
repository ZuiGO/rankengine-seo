...
async def _report_html(job_id: str):
    ...
    if not versions:
        version_html = '<p class="muted">No content changes applied yet. Approve SEO action items on the Actions tab to generate improved content.</p>'
    else:
        rows = []
...
async def _render_pdf(html: str) -> bytes | None:
    ...
    try:
        ...
        get_logger("reports").error("PDF export failed: %s", e)
        return None
...
async def download_report_pdf(job_id: str):
    ...
    pdf_bytes = await _render_pdf(await _report_html(job_id))
...
async def email_report_route(job_id: str, req: EmailReportRequest):
    to = (req.to or "").strip()
